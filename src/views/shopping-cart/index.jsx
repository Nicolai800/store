...
export const ShoppingCart = () => {
  ...
  const filteredItems = allItems.filter((item) => shopingCartItems[item.id] > 0);
  const totalSum = filteredItems.reduce((sum, item) => {
    return (
      sum + shopingCartItems[item.id]*(item.discont_price === null ? item.price : item.discont_price)
    );
  }, 0);
  ...
  if (cardCounter === 0) {
    return (
      <>
        ...
        >
          <h2>Shoping Cart</h2> <div className={styles.lineWrapper}><div className={styles.line}></div><Link to="/"className={styles.titleLink}>Back to the store</Link></div>
        </div>
        <div className={cn(styles.emptyBasket, {
            [styles.dark]: theme === "dark",
          })}>
    ...
    return (
      <>
        ...
        <div className={cn(styles.shoppingCartWrapper, {
            [styles.dark]: theme === "dark",
          })}>
          ...
          <div className={cn(styles.orderForm, {
            [styles.dark]: theme === "dark",
          })}>
            <h3>Order Details</h3>
            <div className={styles.orderPrais}>
              ...
              <div className={styles.total}>
                <p>Total</p><span className={cn(styles.totalSum, {
            [styles.dark]: theme === "dark",
          })}>{totalSum.toFixed(2)}$</span>
              </div>
              <div className={cn(styles.orderInputs, {
            [styles.dark]: theme === "dark",
          })}>
                <input
              type="text"
        ...
        {isModalOpen && <OrderModal onClose={onToggleModal} />}
      </>
    );
  }
};
